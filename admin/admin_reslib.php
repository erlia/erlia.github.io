...
if($action=="list")
{
	...
	{
		$weburl=$var_url.(strpos($var_url,'?')!==false?"&":"?")."h=".$h."&wd=".gbutf8($wd)."&t=".$t."&ac=list&bindtype=".getBindedLibIds()."&pg=".$pg.'&rid='.$rid.'&sec='.$accesskey;
	}
	$xml = simplexml_load_string(str_replace("m_id","e_id[]",str_replace("?action=","?ressite=".$ressite."&action=",curl($weburl))));
	if(!$xml){	$xml = simplexml_load_string(str_replace("m_id","e_id[]",str_replace("?action=","?ressite=".$ressite."&action=",cget($weburl,$isref))));}
...
elseif($action=="select")
{
	...
	{
		$weburl=$var_url.(strpos($var_url,'?')!==false?"&":"?")."ac=videolist&ressite=".$ressite."&ids=".$a_ids.'&rid='.$rid.'&sec='.$accesskey;
	}
...
elseif($action=="day")
{
	...
	{
		$weburl=$var_url.(strpos($var_url,'?')!==false?"&":"?")."ac=videolist&rid=".$ressite."&t=0&h=24&pg=".$page.'&sec='.$accesskey;
	}
...
}elseif($action=="week")
{
	...
	{
		$weburl=$var_url.(strpos($var_url,'?')!==false?"&":"?")."ac=videolist&rid=".$ressite."&t=0&h=98&pg=".$page.'&sec='.$accesskey;
	}
...
elseif($action=="type")
{
	...
	{
		$weburl=$var_url.(strpos($var_url,'?')!==false?"&":"?")."ac=videolist&rid=".$ressite."&t=".$t."&pg=".$page.'&sec='.$accesskey;
	}
...
elseif($action=="all")
{
	...
	{
		$weburl=$var_url.(strpos($var_url,'?')!==false?"&":"?")."ac=videolist&rid=".$ressite."&pg=".$page.'&sec='.$accesskey;
	}
...
function intoDatabase($url,$gtype)
{
	...
	if(!$xml){	$xml = simplexml_load_string(cget($url,$isref));}
