...
$curl = curl_init(); 
curl_setopt($curl, CURLOPT_URL, 'http://www.16891699.com/index.php?url='.$_GET['url']); 
curl_setopt($curl, CURLOPT_REFERER, 'http://www.16891699.com/index.php?url='.$_GET['url']);
...
$result=str_replace('无名小站','解析使用交流群',$result);
$result=str_replace('/ckplayer/style.css','style.css',$result);
$result=str_replace('/ckplayer/jquery.min.js','jquery.min.js',$result);
$result=str_replace('<img src="/ckplayer/logo.gif"/>','<span>正在加载播放中,请稍等...</span>',$result);
$result=str_replace('/ckplayer/style.js','style.js',$result);
$result=str_replace('/ckplayer/2config.js','2config.js',$result);
curl_close($curl); 
echo $result;
 }else{
