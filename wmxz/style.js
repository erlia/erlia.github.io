...
function playad() {
    if (WMXZ_WANG.ads) {
        ...
        $('#loading').hide();
        var ads = WMXZ_WANG.ads.split('|');
        var adhtml = '<a href="' + ads[1] + '" target="_blank"><img src="' + ads[0] + '"></a><span>【播放等待<b>' + ads[2] + '</b>秒后开始】</span>';
...
function vod_player(data) {
    var autoplay = WMXZ_WANG.play == 1 ? 'autoplay="autoplay"': '';
    if (data['ext'] == 'link') {
        ...
        if (data['ext'] == 'm3u8' || data['ext'] == 'm3u8_list') {
            var flashvars = {
                f: WMXZ_WANG.webpath + 'ckplayer/m3u8.swf',
                a: data['url'],
                c: 0,
                s: 4,
                lv: 0,
        ...
        CKobject.embedSWF(WMXZ_WANG.webpath + 'ckplayer/ckplayer.swf', 'a1', 'ckplayer_a1', '100%', '100%', flashvars, params);
    }
    $('#loading').hide();
    $('#a1').show();
}
...
function get_play(up) {
    $.get("https://data.video.iqiyi.com/v.f4v",
    function(cdnip) {
